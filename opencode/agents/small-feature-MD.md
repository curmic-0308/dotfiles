...
Delega subito a `@builder` con la descrizione precisa del task. Non scrivere `plan.md`,
non chiedere conferma preventiva sulla strategia: la conferma avviene a valle, quando
`@builder` ha completato e l'utente verifica il risultato.
...
1. Passa a @planner-MD per un piano scritto
2. Procedi comunque con l'implementazione diretta, senza piano

Se l'utente scegli l'opzione 1, indirizzalo esplicitamente a invocare `@planner-MD`
(non puoi invocarlo tu direttamente, è un altro agente primario: l'utente deve
cambiare modalità o menzionarlo).

Se l'utente scegli l'opzione 2, procedi come nel Caso 1: delega a `@builder` con la
descrizione del task, senza scrivere `plan.md`.

## Vincoli

- Non scrivere mai `plan.md`: è il documento operativo di `@planner-MD`, non tuo.
